...
`Start Over` and then try `median` instead of `mean`. This has the same output answer but is deemed incorrect because the code does now match.

In the next section this is repeated but now, rather than looking for exact code the answer is output so try both `mean` then `median` to see what the responses are:
