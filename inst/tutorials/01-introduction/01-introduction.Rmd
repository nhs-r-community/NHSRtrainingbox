...
[Slides](https://nhs-r-community.github.io/intro_r/01-workshop_intro.html) for the introduction course are referenced in this package for further information or to visualise RStudio with screenshots. The slides in the introduction session include links to various examples of how R is used, including Shiny apps, heat map visualisations and a Twitter dashboard. 

### Learning goals

The goal of this section is to introduce some important features of RStudio and whilst there is a lot more that RStudio is capable of, there are often updated versions with new and useful features, the following should get you started. [Presentation slide 22](https://nhs-r-community.github.io/intro_r/01-workshop_intro.html#22) onwards in this section have screenshots to help locate parts of RStudio and links to other resources or sources. The following {learnr} section can be followed without the slides.
...
# Try commenting out the median code (the shortcut won't work in this panel)
median(1:10) + 
  mean(1:10)
```

```{r comment-solution}
# Try commenting out the code for median
# median(1:10) + 
  mean(1:10)
```
...
grade_this({
  if (identical(.result, .solution)) {
    pass("Great work! You commented out the median() code so only the mean() ran")
  }
  fail("Hint! You need to comment out the median() code so only the mean() runs")
})
```
...
There are 3 ways to run code in RStudio, a button, a menu selection and a keyboard short cut key. For any of these options the cursor needs to be somewhere in the code you want to run. Unlike other programs you may be familiar with, not all lines of code need to be selected to run (although this is very handy if you only want to run part of the code to see if it works). If, for example, you have two things to run:
...
Packages can also be downloaded from GitHub repositories and often require another package called {devtools} or {remotes} to install and many of the GitHub READMEs give the code to use. For example:
...
The commented out first line needs to be uncommented but once you have {remotes}/{devtools} loaded this line is no longer needed.  
...
Not all organisations allow packages to be downloaded by anywhere other than CRAN due to security. The issue with this is that many useful packages are in development or change too frequently to be included. Some packages are also hosted on rOpenSci like UKHSA's {fingertipR} which are only available through GitHub.
...
# Complete this code. Note this doesn't have quotations
___(tidyverse)
...
```{r quotations, exercise = TRUE}
# Put quotations around the appropriate word. Try double clicking on the word and typing the opening quotation.

install.packages(tidyverse)
```

```{r quotations-solution}
...
grade_this({
  if (identical(.result, .solution)) {
    pass("Great work! You removed all the uppercase letters from the package names")
  }
})
